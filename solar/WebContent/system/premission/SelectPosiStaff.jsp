...
<%
if(ServletUtil.get(request,"add").length() >0)
{
	%>
	<$:A service="addPosiStaffs" dynamic="false">
		<$:P name="posi_no" from="request"/>
		<$:P name="staffnos" from="request"/>
	</$:A>
	<$:M dtype="1"/>
...
<form method="post" name="posi_staffs">
	<input type=hidden name="staffnos" value=''>
...
if(staff_nos!="")
{
   var staffnos =staff_nos.split(",");
	var oStaffNos =document.getElementsByName("STAFF_NO");
	for(i=0; i<oStaffNos.length; i++)
	{
		var vstaff =oStaffNos[i].value;
...
function onSelectStaff(handlename)
{
	...
	for(i=0; i<handle.length; i++)
	{
		if(handle[i].checked)
		{
			...
			if(staffnos =="")
			{
				staffnos =savalue[1].substring(1);
	...
	var s=new Server("addPosiStaffs");
	s.setParam("staffnos", staffnos);
	s.setParam("posi_no", $E.getParameter("posi_no"));
