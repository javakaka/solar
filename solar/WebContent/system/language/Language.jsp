...
<title><cc:message key="framework.nav.language" /></title>
...
<body>
	<div class="path">
		<cc:message key="framework.nav.index" /> &raquo;<cc:message key="framework.nav.language" />
		<span><cc:message key="admin.page.total" args="${page.total}"/></span>
	</div>
	<form id="listForm" action="Language.do" method="get">
		<div class="bar">
			...
			<div class="menuWrap">
				...
				<div class="popupMenu">
					<ul id="searchPropertyOption">
						<li>
							<a href="javascript:;" <c:if test="${page.searchProperty == 'LAN_NO'}"> class="current"</c:if> val="LAN_NO"><cc:message key="framework.language.lan_no" /></a>
						</li>
						<li>
							<a href="javascript:;" <c:if test="${page.searchProperty == 'LAN_NAME'}">class="current"</c:if> val="LAN_NAME"><cc:message key="framework.language.lan_name" /></a>
						</li>
					</ul>
				</div>
			</div>
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="LAN_NAME">语言名称</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="LAN_STATE">状态</a>
				</th>
				<th>
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				<tr>
					<td>
						<input type="checkbox" name="ids" value="${row.LAN_NO}" />
					</td>
					<td>
						${row.LAN_NO}
					</td>
					<td>
						<span title="${row.LAN_STATE}">${row.LAN_NAME}</span>
					</td>
					<td>
						<c:choose>
					...
					<td>
						<a href="edit.do?LAN_NO=${row.LAN_NO}"><cc:message key="admin.common.edit" /></a>
					</td>
				</tr>
			</c:forEach>
		</table>
