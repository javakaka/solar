	...
	<form id="listForm" action="list.do" method="get">
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="AVATAR">头像</a>
				</th>
				<th>
				...
				<th>
					<a href="javascript:;" class="sort" name="FROM_USER">微信</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="ID_CARD_NO">身份证</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="IS_AUTH">认证</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="SUB_STATE">关注</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="CREATE_TIME">注册时间</a>
				</th>
				<th>
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				<tr>
					...
					<td>
						<span title="${row.ID_CARD_NO}">${row.ID_CARD_NO}</span>
					</td>
					<td>
						<c:choose>
							<c:when test="${row.IS_AUTH == 1}">已认证</c:when>
							<c:when test="${row.IS_AUTH == 0}">未认证</c:when>
							<c:otherwise>未认证</c:otherwise>
						</c:choose>
					</td>
					<td>
						<c:choose>
							<c:when test="${row.SUB_STATE == 0}">未关注</c:when>
							<c:when test="${row.SUB_STATE == 1}">已关注</c:when>
							<c:when test="${row.SUB_STATE == 2}">已取消关注</c:when>
							<c:otherwise>未关注</c:otherwise>
						</c:choose>
					</td>
					<td>
						${row.CREATE_TIME}
