		...
		<table id="listTable" class="list">
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${row.PAY_TYPE == 1}">微信</c:when>
							<c:when test="${row.PAY_TYPE == 2}">支付宝</c:when>
							<c:otherwise>--</c:otherwise>
						</c:choose>
					</td>
					<td>
						<c:choose>
							<c:when test="${row.STATE == 1}">待付款</c:when>
							<c:when test="${row.STATE == 2}">已付款</c:when>
							<c:when test="${row.STATE == 3}">已到账</c:when>
							<c:when test="${row.STATE == 0}">已退货</c:when>
							<c:otherwise>--</c:otherwise>
						</c:choose>
					</td>
					<td>
...
{
	var url ="<%=basePath%>/paimaipage/platform/order/item/list.do";
