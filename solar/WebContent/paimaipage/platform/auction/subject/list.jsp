	...
	<form id="listForm" action="list.do" method="get">
			...
			<div class="menuWrap">
				...
				<div class="popupMenu">
					<ul id="searchPropertyOption">
						...
						<li>
							<a href="javascript:;" <c:if test="${page.searchProperty == 'START_TIME'}">class="current"</c:if> val="START_TIME">起拍日期</a>
						</li>
					</ul>
				</div>
			</div>
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="SESSION_NAME">名称</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="SUMMARY">简介</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="PRE_START_TIME">预展开始</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="PRE_END_TIME">预展结束</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="START_TIME">拍卖开始</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="END_TIME">拍卖结束</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="LOOK_NUM">围观</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="SIGN_NUM">报名</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="PRICE_NUM">出价</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="GOODS_NUM">拍品数量</a>
				</th>
				<th>
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				<tr>
					...
					<td>
						<span title="${row.PRE_START_TIME}">${row.PRE_START_TIME}</span>
					</td>
					<td>
						<span title="${row.PRE_END_TIME}">${row.PRE_END_TIME}</span>
					</td>
					<td>
					...
					<td>
						<span title="${row.LOOK_NUM}">${row.LOOK_NUM}</span>
					</td>
					<td>
						<span title="${row.SIGN_NUM}">${row.SIGN_NUM}</span>
					</td>
					<td>
						<span title="${row.PRICE_NUM}">${row.PRICE_NUM}</span>
					</td>
					<td>
					...
					<td>
						<c:choose>
							<c:when test="${row.STATE == -1}">已删除</c:when>
							<c:when test="${row.STATE == 0}">已结束</c:when>
							<c:when test="${row.STATE == 1}">未开始</c:when>
							<c:when test="${row.STATE == 2}">拍卖中</c:when>
