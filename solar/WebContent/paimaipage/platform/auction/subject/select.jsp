	...
	<form id="listForm" action="select.do" method="get">
		...
		<table id="listTable" class="list">
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				<tr>
					<td>
						<input type="radio" name="ids" value="${row.ID}" title="${row.SESSION_NAME}" <c:if test="${id == row.ID}">checked</c:if>/>
					</td>
					<td>
...
function selectTarget(){
	...
	window.parent.setSelectedSession(selectedId,target);
...
function addEventHandlers()
{
	...
	if(typeof is_window_bottom_existed =="undefined" || is_window_bottom_existed == null || is_window_bottom_existed == "")
	{
		...
		b_div.innerHTML="";
		var html="<div style=\"float:right;\"><input type=\"button\" name=\"submitBtn\" id=\"submitBtn\" value=\"确定\" class=\"button\" onclick=\"sessionIframeSelectTarget()\"/></div>";
