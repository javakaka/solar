	...
	<form id="listForm" action="select.do" method="get">
			...
			<div class="menuWrap">
				...
				<div class="popupMenu">
					<ul id="searchPropertyOption">
						<li>
							<a href="javascript:;" <c:if test="${page.searchProperty == 'TYPE_NAME'}"> class="current"</c:if> val="TYPE_NAME">名称</a>
						</li>
					</ul>
				</div>
			</div>
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="TYPE_NAME">名称</a>
				</th>
				<th>
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				<tr>
					<td>
						<input type="radio" name="ids" value="${row.ID}" title="${row.TYPE_NAME}" <c:if test="${id == row.ID}">checked</c:if>/>
					</td>
					<td>
...
function selectTarget(){
	...
	window.parent.setSelectedType(selectedId,target);
...
function addEventHandlers()
{
	...
	if(typeof is_window_bottom_existed =="undefined" || is_window_bottom_existed == null || is_window_bottom_existed == "")
	{
		...
		b_div.innerHTML="";
		var html="<div style=\"float:right;\"><input type=\"button\" name=\"submitBtn\" id=\"submitBtn\" value=\"确定\" class=\"button\" onclick=\"typeIframeSelectTarget()\"/></div>";
